             ...
             <ul class="pagination">
                ...
                <li 
                  @if($page==1)
                   class="disabled"
                   @endif
                ><a href="{{route('payments.totalperday')}}?page=1" rel="next">«</a></li>
                @for($i=1;$i<=$pages;$i++)
                  @if($i==$page)
                    <li class="active"><span>{{$i}}</span></li>
                  @else
                    <li><a href="{{route('payments.totalperday')}}?page={{$i}}">{{$i}}</a></li>
                  @endif
                @endfor
                <li
                  ...
                  @if($page==$pages)
                   class="disabled"
                   @endif
                ><a href="{{route('payments.totalperday')}}?page={{$pages}}" rel="next">»</a></li>
            </ul>
            @endif
            </div>
            <div class="box-footer">
